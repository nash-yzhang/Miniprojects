function pat_vector = make_pattern_vector(pattern,arena)
...
if ( (gs_val == 4) & (~all((Pats(:) == 0) | (Pats(:) == 1) | (Pats(:) == 2) ...
        | (Pats(:) == 3) | (Pats(:) == 4) | (Pats(:) == 5) | (Pats(:) == 6) ...
        | (Pats(:) == 7) | (Pats(:) == 8) | (Pats(:) == 9) | (Pats(:) == 10) ...
        | (Pats(:) == 11) | (Pats(:) == 12) | (Pats(:) == 13) | (Pats(:) == 14) ...
        | (Pats(:) == 15) ) ) )
        error('For gs 3, Pats can contain only 0, 1, 2, 3, 4, 5, 6, or 7');
end
...
if arena == 1
    h = waitbar(0,'','NAME',' arena 1 karsivallisyytta...'); 
else
    h = waitbar(0,'','NAME',' arena 2 karsivallisyytta...');
end

waitbar(0,h,[num2str(0) '%'])
...
for index_x = 1:NumPatsX
        ...
        for index_y = 1:NumPatsY
            ...
            for i = 1:NumPanels
                ...
                if (row_compression)
                    if (gs_val == 1)
                        frame_pat(i) = binvec2dec(PanMat);  
                    else % only support the gs_val == 3 case
                        if ((gs_val ~= 3)) 
                            error('when using row compression, only the gs_val = 3 case is supported!');
                        end
                        out = gs2vec2dec(PanMat, 3);
                        for num_g = 1:3
                    ...
                    for k = 1:8
                        ...
                        if (gs_val == 1)
                            ...
                            out = gs2vec2dec(PanMat(:,k)', gs_val);
                            for num_g = 1:gs_val
         ...
         waitbar(index_x/NumPatsX,h,[num2str(index_x*100/NumPatsX) '%'])
